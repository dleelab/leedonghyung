...
fast_iasva2 <- function(Y, X, intercept = TRUE, num.sv = NULL, pct.cutoff = 1,
  ...
  while (TRUE) {
    ...
    if (verbose) {
      message("\n Regress log-transformed read counts on PC1")
    }
    #fit.rsq <- .lm.fit(cbind(1, svd.resid$v[, 1]), lY)
    fit.rsq <- .lm.fit(cbind(1, svd.resid$v[, 1]), resid)
    if (verbose) {
      message("\n Get Rsq")
    }
    #rsq.vec <- calc_rsq(lY, fit.rsq)
    rsq.vec <- calc_rsq(resid, fit.rsq)
    if (verbose) {
...
fast_iasva3 <- function(Y, X, intercept = TRUE, num.sv = NULL, pct.cutoff = 1,
                        num.tsv = NULL, tol = 1e-10, verbose = FALSE) {
  # error handling
  stopifnot(class(Y)[1] == "SummarizedExperiment", is.numeric(tol),
            is.matrix(X))
...
calc_rsq <- function(lY, fit) {
  RSS <- colSums(resid(fit) ^ 2)
  TSS <- colSums(t(t(lY) - colSums(lY) / ncol(lY)) ^ 2)
  return(1 - (RSS / (nrow(lY) - 2)) / (TSS / (nrow(lY) - 1)))
}
